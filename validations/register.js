...
let user = body('user').notEmpty().withMessage('El nombre de usuario no puede quedar vacío').custom(function(user){
...
}).withMessage('Usuario ya registrado').bail().isLength({min:5,max:20}).withMessage('minimo 5 caracteres, maximo 20')

let fullName = body('fullName').notEmpty().withMessage('El nombre de usuario no puede quedar vacío').custom(function(user){
   return db.user.findOne({
    where:{
        user: user
    }}).then(function(data){
